...
;// CONCATENATED MODULE: ./ui/src/pages/transformations/lookup_ff_request.js
/*
Copyright (c) 2023, CloudBlue LLC
All rights reserved.
*/
...
const AVAILABLE_FF_REQUEST_ATTRS = [
  {
    value: 'id',
    label: 'FF Request ID',
  },
  {
    value: 'status',
    label: 'FF Request status',
  },
  {
    value: 'events.created.at',
    label: 'FF Request created at',
  },
  {
    value: 'events.updated.at',
    label: 'FF Request updated at',
  },
  {
    value: 'asset.id',
  ...
  {
    value: 'asset.items.id',
    label: 'Product item IDs',
  },
  {
    value: 'asset.items.global_id',
    label: 'Product item global IDs',
  },
  {
    value: 'asset.items.mpn',
    label: 'Product item MPNs',
  },
  {
    value: 'asset.items.item_type',
...
const createOutputRow = (parent, index, column, parameters, columnConfigs) => {
  ...
  AVAILABLE_FF_REQUEST_ATTRS.forEach(optionData => {
...
const lookupFFRequest = (app) => {
  ...
  app.listen('config', async (config) => {
    ...
    if (hasProduct === true) {
      parameters = await getLookupSubscriptionParameters(stream.context.product.id, 'ff_request');
  ...
  app.listen('save', async () => {
    ...
    try {
      const overview = await validate('lookup_ff_request', data);
      if (overview.error) {
...
/******/ 	var __webpack_exports__ = __webpack_require__.O(undefined, [216], () => (__webpack_require__(601)))
